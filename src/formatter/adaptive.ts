...
/**
 * Format analysis results in adaptive format (Markdown)
...
export function formatAdaptive(
  ...
  lines.push(`**Duration:** ${formatDuration(analysis.durationNanos)} | **Samples:** ${analysis.totalSamples.toLocaleString()}`)
  lines.push('')
  ...
  for (let i = 0; i < topHotspots.length; i++) {
    ...
    lines.push(`${i + 1}. \`${h.name}\` (**${h.selfPercent.toFixed(1)}%**) → [Details](#${drillId})`)
  }
  lines.push('')
  ...
  for (const hotspot of topHotspots) {
    ...
    lines.push(`<a id="${drillId}"></a>`)
    lines.push('')
    lines.push(formatDrillDownSection(hotspot, analysis, sourceDir, includeSource))
    lines.push('')
  }
...
function generateExecutiveSummary(
  ...
  lines.push(`- **Primary bottleneck:** \`${primary.name}\` (**${primary.selfPercent.toFixed(1)}%** of ${profileType === 'cpu' ? 'CPU' : 'allocations'})`)

  // Secondary bottleneck
  if (hotspots.length > 1) {
    const secondary = hotspots[1]
    lines.push(`- **Secondary bottleneck:** \`${secondary.name}\` (**${secondary.selfPercent.toFixed(1)}%**)`)
  }
  ...
  lines.push(`- **Optimization potential:** ${potential} (${appPct.toFixed(0)}% in application code)`)
...
function formatDrillDownSection(
  ...
  if (pathToHotspot.length > 0) {
    lines.push(`**Call path:** ${pathToHotspot.map(p => `\`${p}\``).join(' → ')}`)
  }

  lines.push(`**Self-time:** ${hotspot.selfPercent.toFixed(1)}% (${hotspot.sampleCount.toLocaleString()} samples)`)
  lines.push(`**Type:** ${isNative ? 'Native V8/Node.js function' : 'Application code'}`)
  lines.push('')
...
function generateInsights(
  ...
  if (
    ...
    insights.push('High cumulative time relative to self-time suggests this is a coordinator function')
    insights.push(`Callees account for most time: ${hotspot.callees.slice(0, 3).map(c => `\`${c}\``).join(', ')}`)
  }
